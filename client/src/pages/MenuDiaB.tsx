/**
 * MenuDiaB — Menú para días de descanso
 * Igual que Día A pero con hidratos reducidos (día off)
 */
...
export default function MenuDiaB() {
  const [, navigate] = useLocation();
  const { state } = useNutrition();
  const { distribucionDiaB: dist, alimentos, macros: m, datosPersonales } = state;
  ...
  const hidratosOffTotal = m.hidratosOffGrKg * datosPersonales.peso;
  const hidratosOffKcal = hidratosOffTotal * 4;
  ...
  return (
    <div className="min-h-screen bg-gray-950 p-6">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Menú Día B</h1>
          <p className="text-gray-400 text-sm mt-1">Días de descanso — Hidratos reducidos</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => navigate("/menu-a")}
            className="border-gray-700 text-gray-300 hover:bg-gray-800 bg-transparent">
            <ChevronLeft size={16} className="mr-1" /> Menú A
          </Button>
          <Button onClick={() => navigate("/partido")} className="bg-cyan-500 hover:bg-cyan-400 text-gray-950 font-semibold">
            Día de Partido <ChevronRight size={16} className="ml-1" />
          </Button>
        </div>
      </div>
      ...
      <div className="mb-5 p-4 bg-gray-800/50 border border-gray-700 rounded-xl">
        <p className="text-xs text-gray-400">
          <span className="text-amber-400 font-medium">Día de descanso:</span> Los hidratos se reducen a{" "}
          <span className="text-amber-400 font-bold">{m.hidratosOffGrKg.toFixed(1)} gr/kg</span>{" "}
          ({Math.round(hidratosOffTotal)}g · {Math.round(hidratosOffKcal)} kcal).
          Proteínas y grasas se mantienen igual.
        </p>
      </div>
      ...
      <div className="mb-6 grid grid-cols-4 gap-3">
        {[
          { label: "Kcal", val: Math.round(totalKcal), target: Math.round(m.kcalTotales * 0.85), color: "text-cyan-400" },
          { label: "HC", val: totalHC.toFixed(1), target: Math.round(hidratosOffTotal), color: "text-amber-400" },
      ...
      <div className="space-y-4">
        {comidas.map(({ key, label, emoji }) => {
          ...
          menuComida.alimentos.forEach(a => {
            ...
            if (!alim) return;
            const gr = calcularGramos(alim, a.tipo, a.intercambios);
            cKcal += calcularNutriAlimento(alim, gr).kcal;
          });
          ...
          return (
            <div key={key} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  {cKcal > 0 && <span className="text-xs text-gray-400">{Math.round(cKcal)} kcal</span>}
                      ...
                      {menuComida.alimentos.map((a, idx) => {
                        ...
                        return (
                          <div key={idx} className="flex items-center gap-3 p-2 bg-gray-800/50 rounded-lg">
                            ...
                            <button onClick={() => removeAlimento(key, idx)} className="text-gray-600 hover:text-red-400 transition-colors">
                              <X size={14} />
                            </button>
                          </div>
                        );
